...
class KuisionerLogic extends BaseLogic {
  ...
  var load_get_kuisioner = true.obs;
  ...
  void clickRadioButton(
      String id_value, String pertanyaanCode, String newValue) {
  ...
  void handleSlider(String pertanyaanCode, double newValue) {
    ...
    double baru = newValue /
        10; //simpan value baru dibagi dengan 10 pada hashSemua, bukan hashSlider
    state.hashSlider![pertanyaanCode] = newValue.toString(); //disimpan disini
    state.hashSemua![pertanyaanCode] = baru.toString(); //hashse
    debugPrint(
        "nilai " + pertanyaanCode + " : " + state.hashSemua![pertanyaanCode]!);
    //refresh di view
    update();
  }
  ...
  void getPertanyaan() async {
    ...
    final pertanyaanRespon =
        await getService<PertanyaanService>()?.getPertanyaan();
    //jika hit api gagal
    if (pertanyaanRespon is PertanyaanError || pertanyaanRespon == null) {
      ...
      obxPertanyaan.value.data!.forEach((a) {
        ...
        a.pertanyaan!.pERTANYAAN = state.nomor_pertanyaan!.toString() +
            ". " +
        ...
        if (a.pertanyaan!.iDFORMAT == "3") {
          ...
          if (a.subPertanyaan == null) {
            ...
            if (a.subPertanyaan!.length == 1 &&
                a.subPertanyaan![0].labelPertanyaan == null) {
          ...
          for (var i in a.subPertanyaan!) {
            for (var j in i.templateJawaban!) {
              ...
              choiceModels.add(ChoicechipModel(
                  id: j.iDOPSI,
        ...
        if (a.pertanyaan!.iDFORMAT == "5") {
          if (a.subPertanyaan == null || a.subPertanyaan!.isEmpty) {
            ...
            //displit, supaya bisa masuk column PERTANYAAN_CODE dan VALUE_ID
            a.subPertanyaan!.forEach((sub) {
              state.hashSlider![a.pertanyaan!.pERTANYAANCODE! +
                  ";" +
                  sub.labelPertanyaan!.iDVALUE!] = "10";
              state.hashSemua![a.pertanyaan!.pERTANYAANCODE! +
                  ";" +
                  sub.labelPertanyaan!.iDVALUE!] = "1";
            });
          }
        }
  ...
  void kirimJawabanKuisioner() async {
    ...
    List<SubmitjawabanRequest> requests =
        List<SubmitjawabanRequest>.empty(growable: true);
    ...
    final baseRespon =
        await getService<SubmitjawabanService>()?.doSubmit(sRequest);
    load_get_kuisioner.value = false;
    ...
    if (baseRespon is BaseError || baseRespon == null) {
      ...
      Fungsi.suksesToast(
          "Jawaban kuisioner berhasil dikirimkan. Terima kasih atas partisipasi Anda.");
    }
  }
  ...
  void cekKuisioner() async {
    ...
    final baseRespon = await getService<CekkuisionerService>()
        ?.cekKuisioner(userID!, state.id_antrian);
