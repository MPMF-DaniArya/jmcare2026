...
class PengkiniandataLogic extends BaseLogic {
  ...
  void getDetail() async {
    ...
    state.current_option_kewarganegaraan =
    (auth.data!.kewarganegaraan == null)
        ? state.options_kewarganegaraan[0]
        : state.options_kewarganegaraan[1];
  }
  ...
  void setTanggal(DateTime date){
    state.tecTgllahir!.text = DateFormat("yyyy-MM-dd").format(date);
  ...
  Future pickImage(String jenis, ImageSource sumber) async {
    ...
    if (jenis == "KTP") {
      ...
      final kompres = await FlutterImageCompress.compressWithFile(
          state.imageProfil!.absolute.path,
          quality: 50,
          rotate: 0);
      final a = kompres!.toList();
  ...
  Future pickSelfi() async {
    var image = await state.imagePicker!.pickImage(
  ...
  void dialogJMO(String jenis) async {
    final result =
    await Get.toNamed(Konstan.rute_dialog_jmo, arguments: {"jenis": jenis});
  ...
  void getKodepos() async {
    obsKodepos.value = true;

    final wilayahResponse = await getService<WilayahService>()?.getWilayah("kodepos", state.selected_id_kelurahan);
    if (wilayahResponse is WilayahError || wilayahResponse == null){
      Fungsi.errorToast("Tidak dapat memproses data kodepos!");
    }else{
      final pertama = wilayahResponse!.data!.first!;
      state.tecKodepos!.text = pertama.nama!;
    }
    obsKodepos.value = false;
  }
  ...
  void submit() async {
    ...
    // print("tdtgn " + state.base64_tandatangan);
    ...
    if ( state.signatureGlobalKey!.currentState == null || state.base64_tandatangan.isEmpty || state.base64_tandatangan == "" || state.base64_tandatangan.substring(0,150) == Konstan.tag_tandatangan_kosong.substring(0,150)) {
      Fungsi.errorToast("Tanda tangan belum dilakukan!");
      return;
    }
    print( "ttd " + state.base64_tandatangan);
    ...
    final baseRespon = await getService<PengkiniandataService>()?.doPengkinian(
        ...
        state.tecNPWP!.text,
        state.tecNamaibukandung!.text,
        state.tecPendidikan!.text,
        state.tecStatusnikah!.text,
        state.tecJumlahtanggungan!.text,
        state.tecAlamatkantor!.text,
        state.tecTelpkantor!.text,
        state.cekBersedia ? "1" : "0",
        state.current_option_kewarganegaraan,
        state.base64_ktp,
        state.base64_profil,
        state.base64_tandatangan);
    if (baseRespon is BaseError || baseRespon == null || baseRespon!.code! == "100") {
      Fungsi.errorToast("Error " + baseRespon!.message!);
    } else {
      Get.back();
